...
def run_judge_single(
    ...
    if model in OPENAI_MODEL_LIST:
        ...
        judgment = chat_completion_cohere(
            judge_model, judge_tokenizer, conv, temperature=0, max_tokens=2048
        )
    else:
...
def play_a_match_single(
    match: MatchSingle, output_file: str, judge_model=None, judge_tokenizer=None
):
...
def chat_completion_cohere(
    model, tokenizer, conv, temperature, max_tokens, api_dict=None
):
    output = API_ERROR_OUTPUT
    try:
        messages = conv.to_openai_api_messages()
        ...
        output_ids = gen_tokens[0][len(input_ids[0]) :]
